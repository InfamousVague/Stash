...
- Name: unlock.spec.ts >> Unlock Screen (Native) >> shows unlock screen on launch
- Location: e2e-native/unlock.spec.ts:4:3
...
Error: strict mode violation: locator('input[type="password"]') resolved to 2 elements:
    1) <input value="" class="input" type="password" placeholder="Master password"/> aka getByRole('textbox', { name: 'Master password' })
    2) <input value="" class="input" type="password" placeholder="Confirm password"/> aka getByRole('textbox', { name: 'Confirm password' })
...
- generic [ref=e4]:
  ...
  - generic [ref=e8]:
    - textbox "Master password" [active] [ref=e10]
    - textbox "Confirm password" [ref=e12]
  - button "Create Vault" [ref=e14] [cursor=pointer]
```
