...
export function StashSetupBanner({ projectId, projectName, onSetupComplete }: StashSetupBannerProps) {
  ...
  const handleSetup = async () => {
    ...
    try {
      ...
      setInitialized(true);
      toast.success(t('stashSetup.success', { name: projectName }));
      onSetupComplete?.();
    } catch (e) {
  ...
  return (
    <div className="stash-setup-banner">
      ...
      <div className="stash-setup-banner__actions">
        <Button variant="primary" size="sm" icon={check} onClick={handleSetup} disabled={setting}>
          {setting ? t('stashSetup.setting') : t('stashSetup.enable')}
        </Button>
